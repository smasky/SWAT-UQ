...
projectPath = "E:/wuhai_swat/TxtInOut_qianli"  # your SWAT Project Path
workPath = "E:/wuhai_swat/" # your Work Path
exeName = "SWAT_64rel.exe" # the name of swat.exe you want to run
paraFileName = "paras.par" # the parameter file you prepared
evalFileName = "eval.evl" # the evaluation file you prepared

def userObjective(attr):
   
   Y = attr['objSeries'][1][1]['sim']
...
problem = SWAT_UQ(
   ...
   userObjFunc=userObjective,
   optType = 'max'
)

# from UQPyL.optimization import GA

# ga = GA(maxFEs = 5000, verboseFlag=True, saveFlag=True, verboseFreq=1)
...
a = time()
Y = problem.apply_parameters(X[0:1,:])
b =time()
...
# np.savetxt("Y.txt", Y['objSeries'][1][1]['sim'])
