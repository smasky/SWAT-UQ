...
# Read sampling parameters
{
  ...
  A <- read.csv("single_params.csv",header = T)
  params <- A[1,]
...
# Running SWAT preparation tool & SWAT
{
  ...
  # system(prefname, show.output.on.console = F, invisible = T)
  ...
  if(!is.null(model.in.rows)){
    ...
    # else
    #   file.copy(dir("xgeswat\\Scenarios\\Default\\TxtInOut\\Backup",full.names=T),"xgeswat\\Scenarios\\Default\\TxtInOut",recursive=F, overwrite = T) ##Copy out from into backup folder
 
    model.in.file <-as.character(c("xgeswat\\Scenarios\\Default\\TxtInOut\\model.in"))
    
    write.table(model.in.sample, model.in.file, quote=FALSE, row.names=model.in.rows, col.names=FALSE)
  ...
  # save land use contribution
  hruResult <- hru.data[(nrow(hru.data)-hru.cnt+1):nrow(hru.data),]
  TN <- NULL
  TP <- NULL
  labels <- NULL
  
  for (i in 1:hru.cnt) {
    single_result <- hruResult[i,]
    # search the position of LandUse type
    flag <- FALSE
    if(!is.null(labels)){
      for (j in 1:length(labels)) {
        if(single_result[1,1] == labels[j]){
          flag <- TRUE
          break
        }
      }
    ...
    if(flag){
      TN[j] = TN[j] + single_result[1,57] + sum(single_result[1,60:63])
      TP[j] = TP[j] + sum(single_result[1,58:59]) + sum(single_result[1,64:65])
    }else{
      labels <- c(labels,single_result[1,1])
      TN =c(TN, single_result[1,57] + sum(single_result[1,60:63]))
      TP =c(TP, sum(single_result[1,58:59]) + sum(single_result[1,64:65]))
    }
  }
  TN <- t(TN)
  TP <- t(TP)
  colnames(TN) <- labels
  colnames(TP) <- labels
  write.csv(TN,"Landuse_TN.csv", row.names=F) 
  write.csv(TP,"Landuse_TP.csv", row.names=F) 
  ...
  if(Flow){
    fl <- t(simResult[7])
    write.table(fl,"single_sim_result_Flow.dat",col.names = F, row.names = F,append = F)
  }
  
  if(Sed){
    se <- t(simResult[11])
    write.table(se,"single_sim_result_Sed.dat",col.names = F, row.names = F,append = F)
  }
  
  if(NO3){
    no <- t(simResult[18])
    write.table(no,"single_sim_result_NO3.dat",col.names = F, row.names = F,append = F)
  }
  
  
  if(NH4){
    nh <- t(simResult[20])
    write.table(nh,"single_sim_result_NH4.dat",col.names = F, row.names = F,append = F)
  }
  
  if(DO){
    do <- t(simResult[30])
    write.table(do,"single_sim_result_DO.dat",col.names = F, row.names = F,append = F)
  }
  
  if(TN){
    tn <- t(simResult[48])
    write.table(tn,"single_sim_result_TN.dat",col.names = F, row.names = F,append = F)
  }
  
  
  if(TP){
    tp <- t(simResult[49])
    write.table(tp,"single_sim_result_TP.dat",col.names = F, row.names = F,append = F)
  }
